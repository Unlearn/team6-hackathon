      ...
      <CardHeader class="space-y-6 pb-8">
        <div class="space-y-3">
          <div class="flex items-center justify-between text-sm text-muted-foreground mb-2">
            ...
            <span class="font-medium">80% Complete</span>
          </div>
          <Progress :model-value="80" class="h-3" />
        </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      sessionStorage.setItem('wizardStep4', JSON.stringify({
        tradeIds: selectedTrades.value.map(t => t.id)
      }))
      // Navigate to step 5 immediately
